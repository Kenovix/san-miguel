		...
		if($i%2)
			...
			for($z=0;$z<$this->NumCols;$z++){
				...
				<input type="hidden" name="TxtBuscarFactura" value="'.$idFactura.'">
						...
						<input type="number" name="TxtCantidadDev" value="'.$datos[7].'" min="0" max="'.$datos[7].'" step="any" style="font-size:1em; color:#333; font-family:Arial, sans-serif;">
					...
					for($z=0;$z<$h;$z++){
						...
						if($z==2 or $z==15 or $z==17 or $z==19 or $z==9)	{
							if($z==2){
								...
								Editar:<input type="submit" name="BtnEditarCantidad" value="Editar"></form>
								<form name="formAgregaItems'.$registros2[0].'" method="POST" action="VerInventariosVender.php" target="_self" >
								<input type="hidden" name="CbVentasActivas" value="'.$VentasActivas.'">
								<input type="hidden" name="BorrarItem" value="'.$registros2[0].'">
								Borrar:<input type="submit" name="BtnBorrarItem" value="Borrar"></form></td>');
								...
								Editar:<input type="submit" name="BtnEditarCantidad" value="Editar"></form></td>');
							}else{
...
//$registros1=mysql_fetch_array($registros);	
			...
			if(mysql_num_rows($registros)){
				...
				while($registros2=mysql_fetch_array($registros)){
					...
					$Columnas[2]="Num_Documento_Interno";	$Valores[2]=$NumVenta;
					...
					$Columnas[18]="Debito";					$Valores[18]=round($TotalVenta);
					$Columnas[19]="Credito";				$Valores[19]="0";
					$Columnas[20]="Neto";					$Valores[20]=round($TotalVenta);
					...
					$Valores[18]="0";
					$Valores[19]=round($TotalVenta-$registros2["Impuestos"]); 			//Credito se escribe el total de la venta menos los impuestos
					$Valores[20]=round($Valores[9]*(-1));  											//Credito se escribe el total de la venta menos los impuestos
					...
					if($registros2["Impuestos"]<>0){
						...
						$Valores[16]=$NombreCuenta;
						$Valores[18]="0";
						$Valores[19]=round($registros2["Impuestos"]); 			//Credito se escribe el total de la venta
						$Valores[20]=round($registros2["Impuestos"]*(-1));  	//para la sumatoria contemplar el balance
					...
					if($registros2["Descuento"]<>0){
						...
						$Valores[15]=$CuentaPUC;
						$Valores[16]=$NombreCuenta;
						$Valores[18]=$DescuentoAntes;
						$Valores[19]="0"; 			//Credito se escribe el total de la venta
						$Valores[20]=$DescuentoAntes;  	//para la sumatoria contemplar el balance
						...
						*/
						...
						$Valores[16]=$NombreCuenta;
						$Valores[18]=0;
						$Valores[19]=$DescuentoTotal; 			//Credito se escribe el total de la venta
						$Valores[20]=$DescuentoTotal*(-1);  	//para la sumatoria contemplar el balance
					...
					$Valores[15]=$CuentaPUC;
					$Valores[16]=$NombreCuenta;
					$Valores[18]=$TotalCostoP;//Debito se escribe el costo de la mercancia vendida
					$Valores[19]="0"; 			
					$Valores[20]=$TotalCostoP;  	//para la sumatoria contemplar el balance
					...
					$Valores[16]=$NombreCuenta;
					$Valores[18]="0";
					$Valores[19]=$TotalCostoP;//Credito se escribe el costo de la mercancia vendida			
					$Valores[20]=$TotalCostoP*(-1);  	//para la sumatoria contemplar el balance
			...
			///////////////////////Por fuera del ciclo para hacerlo por la venta en general
				...
				if($Comision<>0 and $TipoVenta=="Contado" and $Comisionista<>"NO" ){	
					...
					$Valores[16]=$NombreCuenta;
					$Valores[18]="0";
					$Valores[19]=round($Comision);			//Credito se escribe el costo de la comision			
					$Valores[20]=round($Comision*(-1));  	//para la sumatoria contemplar el balance
					...
					$Valores[15]=$CuentaPUC;
					$Valores[16]=$NombreCuenta;
					$Valores[18]=round($Comision);			//Debito se escribe el costo de la comision
					$Valores[19]="0";		
					$Valores[20]=round($Comision);  	//para la sumatoria contemplar el balance
