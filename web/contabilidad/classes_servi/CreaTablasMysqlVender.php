...
///////////////////////////////////////////////////////////////////
	...
	public function SumeColumnaEsp($NombreColumnaSuma, $NombreColumnaFiltro,$filtro)
  {
	...
	$sql="SELECT SUM($NombreColumnaSuma) AS suma FROM $this->tabla WHERE $NombreColumnaFiltro = '$filtro'";
	//print($sql);
	$reg=mysql_query($sql, $this->con) or die('no se pudo obtener la suma en SumeColumna: ' . mysql_error());
	$reg=mysql_fetch_array($reg);
...
public function AgregaPreventa($fecha,$Cantidad,$idVentaActiva,$idProducto,$idCliente,$idUsuario)
  {
	...
	$reg=mysql_query("select * from fechas_descuentos where Fecha = '$fecha'", $this->con) or die('no se pudo consultar los valores de fechas descuentos en AgregaPreventa: ' . mysql_error());
	$reg=mysql_fetch_array($reg);
...
$this->NumCols=$numCols;
	...
	$reg=mysql_query("select * from cotizaciones where NumCotizacion = '$idCotizacion'", $this->con) or die('no se pudo conectar a cotizaciones 2: ' . mysql_error());
		$i=0;
		...
		if($i%2)
			...
			for($z=0;$z<$this->NumCols;$z++){
				...
				$CantDisponible=$datos["Cantidad"]-$datos["Devuelto"];
				print('<td width="200"  style= "border: 0px solid #000000;color:'.$this->fontCeldas.';font: oblique 100% sans-serif bold;background-color:'.$backgroundRow.'">
				...
				<input type="hidden" name="TxtBuscarFactura" value="'.$idFactura.'">
						<textarea type="text" name="TxtObservaciones" value="" placeholder="Observaciones" required></textarea>
						<input type="number" name="TxtCantidadDev" value="'.$CantDisponible.'" min="0" max="'.$CantDisponible.'" step="any" style="font-size:1em; color:#333; font-family:Arial, sans-serif;">
						<input type="hidden" name="ItemVenta" value="'.$datos[0].'">
						
						<input type="submit" name="BtnDevolver" value="Devolver" onclick="devuelve(); return false"></form></td>');
...
public function TableDrawPreVenta($VentasActivas,$idVentaActiva,$idUsuario,$operacion)
 ...
 $registros=mysql_query("SELECT * FROM vestasactivas  
WHERE idVestasActivas='$idVentaActiva'", $this->con) or die('no se pudo conectar: ' . mysql_error());

$DatosVentasActivas=mysql_fetch_array($registros);	
$SaldoFavor=$DatosVentasActivas["SaldoFavor"];
...
if(mysql_num_rows($reg)){//Si existen resultados
   ...
   while($datos=mysql_fetch_array($reg)){
      print('<option value="'.$datos["idProductosVenta"].';'.$datos["Nombre"].';'.$datos["Existencias"].';'.$datos["PrecioVenta"].';'.$datos["Tipo"].';'.$datos["Referencia"].'">');
   }
					...
					for($z=0;$z<$h;$z++){
						...
						if($z==2 or $z==15 or $z==17 or $z==19 or $z==9)	{
							if($z==2){
								...
								$PrecioNormal=round($registros2["PrecioVenta"]/($registros2["IVA"]+1));
								$PrecioMayor=round($registros2["PrecioMayorista"]/($registros2["IVA"]+1));
								print('<td width="400"  style= "border: 0px solid #000000;color:'.$this->fontCeldas.';font: oblique 100% sans-serif bold;background-color:'.$backgroundRow.'">
								<form name="formAgregaItems'.$registros2[0].'" method="POST" action="VerInventariosVender.php" target="_self" >
								<input type="hidden" name="CbVentasActivas" value="'.$VentasActivas.'">
								<input type="hidden" name="EditarCantidad" value="'.$registros2[0].'">
								<input type="hidden" name="'.$registros2[0].'" value="'.$registros2["Cantidad"].'">
								<input type="hidden" name="TxtPrecioMayorista" value="'.$PrecioMayor.'">
								<input type="hidden" name="TxtPrecioNormal" value="'.$PrecioNormal.'">
...
<tr><td width="200" colspan="5" align="right" style= "border: 1px solid #000099;color:'.$this->fontHeader.';font: oblique 100% sans-serif bold;background-color:'.$this->backHeader.'">
SALDO A FAVOR
<td width="200" style= "border: 0px solid #000000;color:'.$this->fontCeldas.';font: oblique 100% sans-serif bold;background-color:'.$this->back1.'"> $'.number_format($SaldoFavor).'</td>


<tr><td width="200" colspan="5" align="right" style= "border: 1px solid #000099;color:'.$this->fontHeader.';font: oblique 100% sans-serif bold;background-color:'.$this->backHeader.'">
<h1>PAGA</h1>
<td width="200" style= "border: 0px solid #000000;color:'.$this->fontCeldas.';font: oblique 100% sans-serif bold;background-color:'.$this->back1.'"><h1> $'.number_format($Paga).'</h1></td>
...
<input type=hidden name="TxtSaldoFavor" value='.$SaldoFavor.'></input>
...
public function RegistreVenta($idVentaActiva,$TipoVenta,$CuentaDestino,$Comision,$Comisionista, $idCliente, $NumVenta,$NumFact,$NumCot)
  {
...
//$registros1=mysql_fetch_array($registros);	
			...
			if(mysql_num_rows($registros)){
				...
				while($registros2=mysql_fetch_array($registros)){
					...
					$TotalVenta=$registros2["TotalVenta"];
					...
					$Columnas[0]="NumVenta";				$Valores[0]=$NumVenta;
					$Columnas[1]="Fecha";					$Valores[1]=$fecha;
					$Columnas[2]="Productos_idProductos";	$Valores[2]=$idProducto;
					$Columnas[3]="Producto";				$Valores[3]=$campos["Nombre"];
					$Columnas[4]="Referencia";				$Valores[4]=$campos["Referencia"];
					$Columnas[5]="Cantidad";				$Valores[5]=$registros2["Cantidad"];
					$Columnas[6]="ValorCostoUnitario";		$Valores[6]=$campos["CostoUnitario"];
					$Columnas[7]="ValorVentaUnitario";		$Valores[7]=$registros2["ValorAcordado"];
					$Columnas[8]="Impuestos";				$Valores[8]=$registros2["Impuestos"];
					$Columnas[9]="Descuentos";				$Valores[9]=$registros2["Descuento"];
					$Columnas[10]="TotalCosto";				$Valores[10]=$registros2["Cantidad"]*$campos["CostoUnitario"]; //Total costo
					$Columnas[11]="TotalVenta";				$Valores[11]=$TotalVenta;
					$Columnas[12]="TipoVenta";				$Valores[12]=$TipoVenta;
					$Columnas[13]="Clientes_idClientes";	$Valores[13]=$idCliente;
					$Columnas[14]="Usuarios_idUsuarios";	$Valores[14]=$registros2["Usuarios_idUsuarios"];
					$Columnas[15]="HoraVenta";				$Valores[15]=$Hora;
					$Columnas[16]="NoReclamacion";			$Valores[16]=$NumFact;
					$Columnas[17]="Cotizaciones_idCotizaciones";			$Valores[17]=$NumCot;
					$Columnas[18]="Especial";				$Valores[18]="NO";
					
					//print_r($CamposVenta);
					
					///////////////////////////Inserto el registro en la tabla ventas
					if($TipoVenta=="Credito" and $idCliente==1)
						exit("<script>alert('Usted está intentando Registrar una venta a credito sin seleccionar un cliente, por favor asignelo a esta venta antes de continuar')</script>");
					else	
						$this->InsertarRegistro($tabla,$NumRegistros,$Columnas,$Valores);
