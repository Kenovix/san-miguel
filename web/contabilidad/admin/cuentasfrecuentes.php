 ...
 */
    ...
    class cuentasfrecuentesPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $field = new StringField('ClaseCuenta');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
            $field = new StringField('UsoFuturo');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
        }
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('cuentasfrecuentesssearch', $this->dataset,
                array('CuentaPUC', 'Nombre', 'ClaseCuenta'),
                array($this->RenderText('CuentaPUC'), $this->RenderText('Nombre'), $this->RenderText('ClaseCuenta')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('cuentasfrecuentesasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('ClaseCuenta', $this->RenderText('ClaseCuenta')));
        }
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            // View column for ClaseCuenta field
            //
            $column = new TextViewColumn('ClaseCuenta', 'ClaseCuenta', $this->dataset);
            $column->SetOrderable(true);
            
            /* <inline edit column> */
            //
            // Edit column for ClaseCuenta field
            //
            $editor = new ComboBox('clasecuenta_edit', $this->GetLocalizerCaptions()->GetMessageString('PleaseSelect'));
            $editor->AddValue('ACTIVOS', $this->RenderText('ACTIVOS'));
            $editor->AddValue('PASIVOS', $this->RenderText('PASIVOS'));
            $editColumn = new CustomEditColumn('ClaseCuenta', 'ClaseCuenta', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $column->SetEditOperationColumn($editColumn);
            ...
            $editor->AddValue('PASIVOS', $this->RenderText('PASIVOS'));
            $editColumn = new CustomEditColumn('ClaseCuenta', 'ClaseCuenta', $editor, $this->dataset);
            $editColumn->SetInsertDefaultValue($this->RenderText('ACTIVOS'));
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $column->SetInsertOperationColumn($editColumn);
        ...
        {
            $result = new Grid($this, $this->dataset, 'cuentasfrecuentesGrid');
    ...
    try
    {
        $Page = new cuentasfrecuentesPage("cuentasfrecuentes.php", "cuentasfrecuentes", GetCurrentUserGrantForDataSource("cuentasfrecuentes"), 'UTF-8');
        $Page->SetShortCaption('Cuentasfrecuentes');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Cuentasfrecuentes');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("cuentasfrecuentes"));
