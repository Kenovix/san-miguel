 ...
 */
    ...
    class cuentasPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $this->dataset->AddField($field, false);
            $field = new StringField('GupoCuentas_PUC');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
        }
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('cuentasssearch', $this->dataset,
                array('idPUC', 'Nombre', 'Valor', 'GupoCuentas_PUC'),
                array($this->RenderText('IdPUC'), $this->RenderText('Nombre'), $this->RenderText('Valor'), $this->RenderText('GupoCuentas PUC')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('cuentasasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('idPUC', $this->RenderText('IdPUC')));
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('GupoCuentas_PUC', $this->RenderText('GupoCuentas PUC')));
        }
        ...
        {
            $result = new Grid($this, $this->dataset, 'cuentasGrid');
    ...
    try
    {
        $Page = new cuentasPage("cuentas.php", "cuentas", GetCurrentUserGrantForDataSource("cuentas"), 'UTF-8');
        $Page->SetShortCaption('Cuentas');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Cuentas');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("cuentas"));
