 ...
 */
    ...
    class cod_municipios_dptosPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('cod_municipios_dptosssearch', $this->dataset,
                array('ID', 'Cod_mcipio', 'Departamento', 'Ciudad'),
                array($this->RenderText('ID'), $this->RenderText('Cod Mcipio'), $this->RenderText('Departamento'), $this->RenderText('Ciudad')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('cod_municipios_dptosasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('Departamento', $this->RenderText('Departamento')));
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            // View column for Departamento field
            //
            $column = new TextViewColumn('Departamento', 'Departamento', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('Departamento_handler');
            
            /* <inline edit column> */
            //
            // Edit column for Departamento field
            //
            $editor = new TextEdit('departamento_edit');
            $editor->SetSize(100);
            $editor->SetMaxLength(100);
            $editColumn = new CustomEditColumn('Departamento', 'Departamento', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $column->SetEditOperationColumn($editColumn);
        ...
        {
            $result = new Grid($this, $this->dataset, 'cod_municipios_dptosGrid');
            ...
            $column->SetInsertOperationColumn($editColumn);
            /* </inline insert column> */
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'Departamento_handler', $column);
    ...
    try
    {
        $Page = new cod_municipios_dptosPage("cod_municipios_dptos.php", "cod_municipios_dptos", GetCurrentUserGrantForDataSource("cod_municipios_dptos"), 'UTF-8');
        $Page->SetShortCaption('Cod Municipios Dptos');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Cod Municipios Dptos');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("cod_municipios_dptos"));
