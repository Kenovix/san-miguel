...
class CssIni{
			 ...
			 <div class="navbar navbar-inverse navbar-fixed-top" >
			  <div class="navbar-inner">
				<div class="container">
				  ...
				  <a class="brand" href="../Menu.php">'.$Title.'</a>
	...
	function CreaBotonAgregaPreventa($Page,$idUser)
  {
	...
	function CrearForm($nombre,$action,$method,$target){
		print('<li><form name= "'.$nombre.'" action="'.$action.'" id="'.$nombre.'" method="'.$method.'" target="'.$target.'" enctype="multipart/form-data">');
	...
	function CrearSelect($nombre,$label,$color,$evento,$required){
		if($required==1)
			$R="required";
		else	
			$R="";
		
		print('<strong style="color:'.$color.'">'.$label.'<select '.$R.' name="'.$nombre.'" onchange="'.$evento.'">');
	...
	/////////////////////Crear una columna con un select
	
	function ColTablaSelectEgreso($NameSelect,$EventoSelect,$Funcion,$TablaSelect,$idTablaSelect,$Filtro,$ItemFiltro){
				
		print('<td>');
		print('<select name="'.$NameSelect.'" '.$EventoSelect.'="'.$Funcion.'" required>');
		
		$this->CrearOptionSelect("","Seleccione un Tipo de Egreso",0);
		$this->CrearOptionSelect("1","Pagar Arriendos",0);
		$this->CrearOptionSelect("2","Servicios Publicos",0);
		$this->CrearOptionSelect("3","Gastos Diversos",0);
		$this->CrearOptionSelect("4","Pagar Prestamos",0);
		$this->CrearOptionSelect("5","Comprar Mercancias",0);
		$this->CrearOptionSelect("6","Comprar Muebles o Equipos de Oficina",0);
		$this->CrearOptionSelect("7","Pagar Contratistas",0);
		$this->CrearOptionSelect("8","Gastos Bancarios",0);
		$this->CrearOptionSelect("9","Pagar IVA",0);
		$this->CrearOptionSelect("10","Pagar CREE o Renta",0);
		$this->CrearOptionSelect("11","Pagar ICA",0);
		print('</select>');
		$this->CrearBoton("BtnAsignarEgreso","Enviar");
		print('</td>');
	...
	function CrearImageLink($page,$imagerute,$target,$Ancho,$Alto){
		print('<a href="'.$page.'" target="'.$target.'"><img src="'.$imagerute.'" style="width: '.$Ancho.'px;height: '.$Alto.'px;"></a>');
	}
	
	/////////////////////Crea un Boton Submit
	
	function CrearBotonFuncion($nombre,$value,$script){
		print('<input type="submit" id="'.$nombre.'"  name="'.$nombre.'" value="'.$value.'" '.$script.' class="btn btn-primary">');
